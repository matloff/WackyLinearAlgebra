...
Consider a matrix-vector product $Mv$. Of course, that means that $v$ is
a column vector $(v_1,v_2,...,v_n)'$.  If $M$ is of size $m \textrm{ x }
n$, then $v$ is $n \textrm{ x } 1$.
...
which, treating the $c_j$ as "numbers," again symbolically, looks like a
$1 \textrm{ x } n$ matrix.  We temporarily pretend it's  $1 \textrm{ x }
n$ even though it's actually $m \textrm{ x } n$. Then
...
looks, once again symbolically like the product of a $1 \textrm{ x } n$
and an $n \textrm{ x } 1$, resulting in a $1 \textrm{ x } 1$, i.e. a
number.
