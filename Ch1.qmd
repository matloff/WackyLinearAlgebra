...
this process.  Note that each row in a transition matrix must sum to 1.
After all, from state i we must go *somewhere*.
...
It can be shown that the vector of long-run state probabilities $\nu$ is
the solution of 
