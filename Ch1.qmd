...
Many graphs are *directed*d, but we will assume undirected
here.]{.column-margin}
...
probabilities. In the network graph case, products are of the form 0x0,
0x1, 1x0 or 1x1. If there is 1 in an entry of the square of the
adjacency matrix, say in row 15, column 19, that means there is a
two-hop path from node 15 to node 19. In this way, using powers of the
matrix, one can determine connectivity of the graph -- node i is
connected to node j if there is a path, of whatever number of hops,
between them -- count the number of paths and so on. The details are
...
could started in state 1, probability $f_1$, then moved to state 5,
probability $a_{15}$, for a total probability of $f_1 a_{15}$. 
Or, we could started in state 2, probability $f_2$, then moved to state 5,
...
Treating $f$ as a column vector, that last expression is
...
So we have a formula for the distribution of $X_j$.

Similarly, setting $d_j$, as a column vector,
to the distribution of $X_j$, 
...
find a high power of P would do this too, but that would involve a lot
of computation, and even then it would not yield the exact answer. We
will return to this in a later chapter. 
