...
### Linear algebra properties of covariance matrices{sec-covprops}

Say we have a random vector $X$, of length $k$, and a nonrandom matrix
$A$ of sise $m \textrm{x} k$. Theh $A X$ is a new random vector $Y$ of $m$
...
\item $Cov(X)$ is a symmetric matrix. This follows from the symmmetry of
the definition.

\item The diagonal elements of $Cov(X)$ are the variances of the random
