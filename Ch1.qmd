...
throughout the book. Readers should spend extra time here, devising some
...
>    3 by the groups' rows and columns.
> 
...
vector $(v_1,v_2,...,v_n)'$.  If $M$ is of size $m \textrm{ x } n$, then
$v$ is $n \textrm{ x } 1$.
...
\textrm{ x } n$ matrix.  We temporarily pretend it's $1 \textrm{ x } n$
even though it's actually $m \textrm{ x } n$, and that the $c_j$ are
...
looks, once again symbolically like the product of $1 \textrm{ x } n$
and $n \textrm{ x } 1$ vectors, resulting in a $1 \textrm{ x } 1$, i.e. a
number.
...
To further illustrate all this, write the above matrix @eq-matrixA as 
...
1 x 2 "matrix" and a 2 x 1 "vector", yielding a 1 x 1 result,
...
The adjacency matrix of an undirected graph is nessarily ________.
...
directed. The function will return an R list, whos
...
\textrm{ x } 1$ and $1 \textrm{ x } 2$. That would give us a 
$2 \textrm{ x } 2$ "matrix"
...
* Model parameters are $p$ (probability of job completion), $q$ 
  (probability of new job arriving) and $m$ (size of the customer
  waiting area).
