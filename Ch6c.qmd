...
\textrm{ x } n$.  Here is our first of several characterizations of
these subspaces:
...
Let $w_1,...,w_q$ be an orthonormal basis for $\mathcal{R}(A)$, where $q
= dim(\mathcal{R}(A))$. By Gram-Schmidt (@sec-gramschmidt), we can
extend that to an orthonormal basis $w_1,...,w_q,w_{q+1},...,w_m$ for
all of $\mathcal{R}^{m}$. The Projection Theorem (@thm-projection) tells
us that any vector in $\mathcal{R}^{m}$ can be written uniquely as a sum
of its projection on to dim(\mathcal{R}(A)) and the subspace spanned by
$w_{q+1},...,w_m$
