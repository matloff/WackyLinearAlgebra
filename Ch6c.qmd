...
Let $r$ denote the rank of $A$. Recall that $r \leq \min(m,n)$.
...
need the notation $W^{\perp}$, for the *orthogonal complement* of a
subspace $W$. It consists of all vectors outside $W$ is the parent
vector space.
...
* Consider the column space first. Write
  ...
  zzz So for any $x$ in $\mathcal{R}^n$, $Ax$ will be in the span of
  $u_1,...u_r$. But what about the converse? zz

  Now ask, is *any* linear combination of $v_1,...v_r$, say $c_1
  v_1+...+c_r v_r$ equal to $Ax$ for some $x$?  The answer is
  yes. Set
  ...
  Since the $v_k$ are an orthonormal set, we have $v_i' v_j = 0$ for all
  cases of $i \neq j$.  Thus 

  $$
  v_i' x = \frac{c_i}{\sigma_i},
  $$

  as desired. So, the column space of $A$ contains

  $$
  D_1 = \textrm{span}(v_1,...,v_r) 
  $$

  Now define the rest of $\mathcal{R}^n$, 

  $$
  D_2 = \textrm{span}(v_{r+1},...,v_n) = D_1^{\perp}
  $$

  If $x$ is in $D_2$, then in @eq-Ax, we will have $v_i'x = 0$
  for $i \leq r$, and so $Ax = 0$. So $x \textrm{ in } D_2$ implies that
  $x$ is *not* in the column space of $A$.

  In other words, $the column space of $A$ is exactly $D_1$.
