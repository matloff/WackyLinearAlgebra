...
These will play a prominent role in this chapter. Say $A$ is of size $m
\textrm{ x } n$. Use 'dim()' to indicate vector space dimension, and
'rank()' for matrix rank.  Here are the first of several
characterizations of these subspaces:
...
Let $w_1,...,w_q$ be an orthonormal basis for $\mathcal{C}(A)$, where $q
= dim(\mathcal{C}(A))$. By Gram-Schmidt (@sec-gramschmidt), we can
extend that to an orthonormal basis $w_1,...,w_q,w_{q+1},...,w_m$ for
all of $\mathcal{R}^{m}$. A vector $x$ is in $dim(\mathcal{N}(A))$ if
and only if it is in the span of $w_{q+1},...,w_m$, thus establishing
the result.
...
2. Compute the eigenvalues $\sigma_1,...,\sigma_n$
   and eigenvectors $q_1,...,q_n$ of $A'A$. Normalize the $q_i$ by
   ...
   ordering for the $q_i$.

3. Since $A'A$ is symmetric and positive-semidefinite, its eigenvalues 
   will be nonnegative and its eigenvectors $q_1,...,q_r$ will be orthogonal as 
   long as $\sigma_1,...,\sigma_r$ are distinct, as is the case for
   numeric data (@sec-prob0). We will have $\sigma_{r+1} = ... =
   \sigma_n = 0$.
