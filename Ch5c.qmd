...
quantity, hopefully each more accurate that the last, eventually
...
Say we have a function $f$ whose root $r$ is of interest to
us.[Our quest for a root may arise for instance in a minimization problem, where we set a derivative to 0 and solve for that root.]{.column-margin}
We might make a series of guesses for $r$ by considering the 
derivative $f'$.  As we try various guesses, the derivative $f'(x)$ 
tells us the direction in which $f$ is decreasing, exactly the 
information we need.

This is illustrated in the figure. Unknown to us, $r = 2$. Our current
guess is $x = 3$. We draw $f'$, i.e. the tangent line to the curve at
our current guess, and temporarily pretend that the line is the curve.
We thus compute the root for the line, which is x = 2.5, and then take
2.5 as our updated guess for the root of the curve.

![root hunting](NewtonRaph.png)

So if the quantity $f$ that we are working with does not have a
derivative, our work is extra difficult.

So, if say **glmnet** fails to converge, what can be done?  For example,
in **glmnet**, the argument **thresh** defines what we mean by "no
longer changes much"; we can decrease or even increase that value.  One
can make sure to center and scale the X data. Tweaking other parameters
may help as well, but in the end, there are no magic solutions. It may
