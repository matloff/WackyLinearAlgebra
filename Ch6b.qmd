...
as we have seen, orthogonal random vectors are statistically
...
to maximize $v'Cov(Q)v$, so we set the derivative of
...
*random-X* setting discussed in @sec-varbhat.  If we center our data
(subtract from each column the mean of that column), then 
