...
## Review:  Random-X Model

In a sidebar in @sec-varbhat, we introduced the notion of *fixed-X*
versus *random-X* regression models. In the former case, the "X" data
has been chosen beforehand by the analyst, and thus is nonrandom. In the
random-X case, the "X" and "Y" are considered to be a sample from some
population, which will be our context here.

As our concrete example, let's once again consider the census data
(@sec-svcensus).
...
```{r}
svc <- regtools::factorsToDummies(svcensus)  # loaded by qeML
head(svc)
```
