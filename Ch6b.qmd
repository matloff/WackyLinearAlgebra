...
$$
v'Cov(X)v + \omega (v'v - 1) 
$$

We also need $u'v = 0$, but we do not need to add a Lagrangian term 
for this, as you will see.

Setting the derivative with respect to $v$ to 0, we have

$$
0 = 2 Cov(X) v + 2 \omega v 
$${#eq-2ndeig}

So $v$ is another eigenvector of $Cov(X)$. If $\omega \neq \gamma$
(which we will not prove here), then $u'v = 0$ comes from @thm-eigortho.
