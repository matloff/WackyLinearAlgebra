...
So we can find $z$ by finding its coordinates $a_i$, as
follows.[Below, keep in mind that $x$ is known but $z$
is not. We desire a method by which we can find $z$
from $x$.]{.column-margin}
...
@sec-gramschmidt will show us how we obtain an orthonormal basis. 

## Matrix Form

We now see how to obtain the matrix $P_W$ in @thm-projdecomp: First,
form a matrix $Q$ in which
row $i$
is $u_i'$ in @eq-orthoproj. Then using matrix
