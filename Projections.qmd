...
In that case, we have the famous relation:

::: {#thm-projdecomp}

Say the vector $x$ is in $\cal R^n$, and $\cal W$ is a subspace of the
...
* $x$ can be uniquely represented as
  ...
  where the two summands are orthogonal to each other.
...
subspace, and jointly $W$ and $W^{\perp}$ span all of $V$.
...
in the vector space $\cal R^n$, where $n$ is the number of our data
...
all $Ab$, as $b$ varies, is a subspace $W$ of $\cal R^n$. In other
...
$x$ in $\cal V$. How do we fine the projection $z$ of $x$ in $\cal W$? 
...
❄️  **Your Turn:** We noted that projection matrices in $\cal R^n$ are
