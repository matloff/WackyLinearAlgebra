...
$W^{\perp}$.
It too is a
subspace, and it will be shown later that jointly $W$ and $W^{\perp}$
span all of $V$.
...
Keep in mind, we are minimizing @eq-rss with respect to $b$. The set of
all $Ab$, as $b$ varies, is a subspace $W$ of $R^n$. In other
words, *we are finding the value of $b$ that makes $Ab$ closest to $S$*.
That means that the minimizing $Ab$ is the projection of $S$ onto $W$.
...
$$
\sum_{i=1}^n A \widehat{\beta} (S_i - A \widehat{\beta}) = 0
$$
...
*normalizing* the vector.]{.column-margin} As noted, an orthogonal basis
in which every vector has length 1 is called *orthonormal*. Recall that
if $x \neq 0$ then $x/||x||$ has length 1, so that any orthogonal basis
can easily be converted to orthonormal.

### Motivation

Our main question:

> Say we have a vector space $V$, a subspace $W$, and a vector
> $x$ in $V$. How do we find the projection $z$ of $x$ in $W$? 

Let $u_1,...,u_k$ be a basis for $W$.[So we are assuming $W$
(but not necessarily $V$ is finite-dimensional. Using proper math,
...
are length 1. Then $<z,u_i> = a_i$, and thus

$$
<x,u_i> = a_i
$$

So, we're done! We wanted to determine the $a_i$ in @eq-aiui, and now we
see that we can easily obtain it by calculating $<x,u_i>$.  So, we have:

> Given: a vector space $V$; a subspace $W$ with orthonormal
> basis $u_1,...,u_k$; and a vector
> $x$ in $V$. Then the projection of $x$ onto $V$ is equal to
> 
> $$
> p = <x,u_1> u_1+...+<x,u_k> 
> $${#eq-orthoproj}
...
@sec-gramschmidt will show us how we obtain an orthogonal basis. 

## Matrix Form

We now see how to obtain the matrix $P_W$ in @thm-projdecomp: Set row $i$
to $u_i'$ in @eq-orthoproj. Then using matrix
partitioning, we havd
...
It will turn out that in $RV(\Omega)_0$, projections take the form
