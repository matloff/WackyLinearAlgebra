...
## Plausibility of Double Descent: the Minimum Norm Solution
...
In our linear regression context @eq-overdetermined,
$B = A'A$, $x=b$ and $q = A'S$.
...
finding $w$ to minimize

$$
||\Sigma w - s||^2
$$ {#eq-newResids}

This version of the original problem is more easily solved, since we are
dealing with a diagonal matrix. Setting the derivative of  @eq-newResids
with respect to $w$ to 0, we have

$$
\Sigma^2 w = \Sigma s
$$

Thus (illustrating for $B$ of rank $r = 2$)
...
(Due to the structure of $\Sigma$ here, it doesn't matter what $w_i, i >
r$ is; let's make them 0s.)

So we see that

$$
w_i = \frac{1}{\sigma_i},  i = 1,..,r
$$

In other words, making use of @eq-diagyourturn,

$$
w = \Sigma^{+} s
$$

Recalling that $V'x = w$ and $U'q = s$, we have
...
solutions have a 0 residual sum of squares, so why not take the one with
smaller variance? It is thus plausible that $x_{min}$ might do better
than the value of $x$ we obtain at interpolation, hence a downward trend
in the curve of predictive accuracy against $p$.  On the other hand, as
$p$ grows even further, $x$ has more and more components, and likely
that minimum-norm $x$ will grow in length at some point, causing an
increase in variance, hence the typical U-shape -- as our *second* U.

In addition, it can be shown that if *gradient descent* is used to
minimize the least-squares problem, the tendency is that the result will
be of minimum norm.
...
condition number against $p$, we again see Double Descent behavior!
...
Double Descent problem? Recall that a large condition number indicates
multicollinearity, which in turn causes high variance in $\widehat{\beta}$, thus impaired predictive ability.
