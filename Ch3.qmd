...
estimating a large number of $\beta_i$ 
...
In a histogram, we are estimating the height of the bins. With 10 bins
...
*How many* predictors can we afford to use with our given data?
...
A special case is the Leaving One Our method, in which the holdout set
...
We can do this by setting $u$ to -1 times row $i$ of $A$, and setting
$v$ to that row as well. Then (the reader should check this with a small
example),
...
just what we need by Sherman-Morrison-Woodbury. Accordingly, 
set $B = A'A$ in that formula, and we are all set.
