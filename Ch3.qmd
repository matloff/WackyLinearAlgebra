...
❄️  **Your Turn:*a Show this. Write $a = (a_1,...,a_k)'$ and find the
...
Consider an $rxr$ matrix $G =$.  For $r = 2$, write $G$ as
...
define submatrices as follows. $G_i$ is the
$(r-1) x (r-1) $submatrix obtained by removing row 1 and column $j$ from
$G$. Then $det(G)$ is defined recursively as

$$
\sum_{i=1}^r (-1)^{i} det(G_i)
$$
