...
matrices, so that $M'u$, say $r \textrm{x} s$ times $s \textrm{ x} 1$,
is conformable matrix multiplication 
while $uM'$ is mathematical nonsense.]{.column-margin}
...
@eq-acova and setting the "hat matrix" $H = (A'A)^{-1} A'$$, we have

$$
Cov(\widehat{\beta}) = H Cov(S|A) H' = H \sigma^2 I H' = \sigma^2 H H' =
\sigma^2 (A'A)^{-1}
$$
