...
[Suppose we *center* the "Y" variable and each "X" variable, i.e.  we
subtract the variable's mean. It can be shown that this is equivalent to
setting $\beta_0$ to 0; the rest of $\beta$ does not change. We then
remove the column of 1s. By the way, the symbolism of the -1 term in our
R code above is meant to allude to "minus," i.e. removing, the $\beta_0$
term.]{.column-margin} Note the column of 1s, needed to pick up the
$\beta_0$ term.
...
shorter people. But it's a simplifying assumption, so it is commonly used.
...
[In some applications, $A$ is actually chosen by an experimenter, so
that it is not random, the so-called *fixed-X* setting. But even in the
random case (*random-X* setting), it is standard to condition on $A$.
When we derive confidence intervals in @sec-confsets, the distinction
won't matter.  By @eq-iterexpect, a conditional confidence interval,
say, at the 95% level also has that level
unconditionally.]{.column-margin} $\widehat{\beta}$ is a random vector,
and thus has a covariance matrix. Using @eq-acova and setting $R =
(A'A)^{-1} A'$, we have
