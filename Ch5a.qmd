...
In @sec-rankprops, we briefly hinted at a possible relation between the
rank of a matrix $A$ and that of $A'A$. It will turn out that the two
ranks are the same. Such information can come in handy. For instance, in
a linear regression model, we need $A'A$ to be invertible. So if in some
setting we already know that $A$ is of nonfull rank, we know that the
same is true for $A'A$; we need not bother finding the rank of the
latter.

This in turn will lead to study of *vector spaces*, a concept that goes
hand-in-hand with matrices in forming the core of linear algebra.
...
\textrm{ x } k$ matrix the rows are members of $\cal R^{m}$ and the
columns are in $\cal R^{k}$.

### The set C(0,1) of all continuous functions on the interval [0,1]

Vector addition and scalar multiplication are done
as functions. If say $u$ is the squaring function and $v$ is the sine
function, then for example $3u$ is the function
...
AF Beardon (*Algebra and Geometry*, 2005, Cambridge).
...
*transformation* (or *map*) $\cal{V}_1$ to $\cal{V}_2$. 
...
that there a some $m \textrm{ x } n$ matrix $A$ that does its work, i.e.
...
$f(v) = w$ then we will also have $f(v+u) = w$ for any $u$ in the null
space of $A$. To see that the second property also does not necessarily
hold, suppose 
...
This can only produce outputs of the form $(a,**,0**$
