...
Often $\widehat{\Sigma}$ will be provided by our application software, such as
with R's **vcov** function to be presented later in this
chapter, but we will need to derive it in this case, using
...
degrees of freedom. Here $r_{\alpha} = k f_{\alpha}$, where $f_{\alpha}$
is the upper-$\alpha$ quantile for the F-distribution with $k$ and $n-k$
