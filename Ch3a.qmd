...
## Review: Confidence Intervals, Standard Errors{#sec-cireview}
...
### Example: Iranian churn data{#sec-ichurn}
...
> If a random variable $T$ can be represented as
> 
> $$
> T = Z_1^1 + ... + Z_k^2
> $$
> 
> where the $Z_i$ are independent N(0,1) random variables, then we see
> that $T$ has a  *chi-square distribution* with $k$ degrees of freedom.

We also need:

::: {#thm-psubw}

Say $X$ has a $k$-dimensional random vector with mean
vector $\mu$ and (invertible) covariance matrix $\Sigma$. Set

$$
W = \Sigma^{-0.5} (X-\mu)
$$

where $\Sigma^{0.5}$ is as in @thm-sqrt. Then $EW = 0$ and 

$$
Cov(W) = I
$$

Furthermore, if $X$ is multivariate normal, then so is $W$
Moreover, $W'W$ has a chi-square distribution with $k$
degrees of freedom.
...
$$
Cov(W) = 
\Sigma^{-0.5} Cov(X) \Sigma^{-0.5} =
\Sigma^{-0.5} \Sigma \Sigma^{-0.5} = I
$$

The claim regarding multivariate normality follows from 
@thm-xnormalthenaxnormal.

Finally, recall that though 0 covariance between two random variables
does not imply that they are statistically independent, the implication
does hold in the case of multivariate normal distributions.  Thus $W'W$,
the sum of the squares of the components of $W$, is indeed the sum of
the squares of $k$ independent N(0,1) random variables, thus chi-square
distributed..

:::


### A confidence set for asymptotically normal estimators

Let's now return to the setting of @sec-cireview. Say we are estimating
a population value $\theta$, a length-$k$ vector, using an estimator
$\widehat{\theta}$ that has an asymptotically normal distribution with
mean vector $\theta$ and covariance matrix $\Sigma$. Denote our estimate
of the latter by $\widehat{\Sigma}$.

Then in @thm-psubw, the statement about the chi-square distribution is
only true asymptotically. However, we will write $=$ instead of $\approx$
below, to avoid clutter.

Say $d_{\alpha}$ is the upper-$\alpha$ quantile of that chi-square
distribution with $k$ degrees of freedom, so that

$$
P[(\widehat{\theta}-\theta)' \widehat{\Sigma}^{-1} 
(\widehat{\theta}-\theta) \leq d_{\alpha}] = 1-\alpha
$$

So, the event

$$
(\widehat{\theta}-\theta)' \widehat{\Sigma}^{-1} (\widehat{\theta}-\theta) 
\leq d_{\alpha}
$$
...
$$
(\theta - \widehat{\theta})' \widehat{\Sigma}^{-1} (\theta-\widehat{\theta}) 
\leq d_{\alpha}
$${#eq-Falpha}

has that probability as well.

> Therefore, the set of all $\theta$ satisfying @eq-Falpha is a
> $100(1-\alpha)$ confidence set for the true population $\theta$.

In the case $k=2$, the set of all $t = (t_1,...,t_k)'$ such that

$$ (t-\theta)' \widehat{\Sigma}^{-1} (t-\theta) = d_{\alpha} $$

is an ellipse in the $(t_1,t_2)$ plane. (If $\Sigma = I$, we have a
circle.) For $k=3$ we have an ellipsoid, a "football," and so on in
higher dimensions. 

So, the above gives us a confidence ellipse for $\theta$ in two
dimensions, a confidence ellipsoid in three dimensions and so on.

The **ellipse** library can draw this for us in the case of linear
regression:
...
inference method, which we now pursue.

### Simultaneous confidence intervals for quantities $a'\theta$

In @sec-rcov, we saw how to form confidence intervals for a quantity
$a'\theta$. We may wish to form several, or even many, such intervals.
...
$\theta$ being *in* the ellipsoid" (i.e. either in the interior or on
the boundary) is at odds with the Langrange formulation, which
stipulates that the min or max values occur on the boundary. But the
latter property is implied by the linearity of $a'\theta$: Consider any
point $q$ that is strictly interior to the ellipsoid, so our objective
function has value $a'q$.  Then there is room for us to move away from
$q$ yet still be inside the ellipsoid, say to the point $u$, yet with
$a'u$ either larger or smaller that $a'q$, depending on the direction we
move in. So the min or max cannot be in the interior.]{.column-margin}
The endpoints of our CI for $a'\theta$ will consist of the latter
quantity's maximum and minimum values subject to $\theta$ being in the
ellipsoid, i.e. subject to the constraint
...
In other words, we solve the Lagrange multiplier problem (@sec-lagrange)

$$
min, max_{\theta, \lambda} ~~
a'\theta + \lambda [(\theta - \widehat{\theta})' 
\widehat{\Sigma}^{-1} (\theta-\widehat{\theta}) - d_{\alpha}] 
$${#eq-scheffeobjftn}
...
respect to $\theta$, we thus have 

$$
0 = a 
+ 2 \lambda 
[\widehat{\Sigma}^{-1} (\theta - \widehat{\theta})]
$$
...
Now substitute for $\theta - \widehat{\theta}$ in the constraint
@eq-constraint:
...
$a'\theta$. Using @eq-sothat, we have 

$$
a'\theta - a'\widehat{\theta} = 
-\frac{1}{2\lambda} a' \widehat{\Sigma} a
$$

Using @eq-lambdaci, our CI for $a'\theta$ is 
...
point is that once we know $\theta$ is in the ellipsoid, then the above
algebraic computations show that for *any* $a$, the quantity $a'\theta$
will be between $a'\widehat{\theta} - \sqrt{d_{\alpha} a'\widehat{\Sigma}
a}$ and $a'\widehat{\theta} + \sqrt{d_{\alpha} a'\widehat{\Sigma} a}$.
Again, this is solely an algebraic property, not a probabilistic one; the only
probabilistic action occurred in $\theta$ being in the ellipsoid.  Thus
@eq-scheffeci will hold for all $a$ simultaneously. 

However, note that we pay a price for the simultaneity, in that
@eq-scheffeci will be wider than the ordinary CI for $a'\theta$, using
@eq-pmse,
...
Consider our Iranian Churn data analysis above in @sec-ichurn, with $k =
12$. Let's find $\sqrt{d_{\alpha}}$, for $\alpha = 0.05$:
