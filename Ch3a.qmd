...
$z_{\alpha}$, the $1-\alpha/2$ quantile of the N(0,1) distribution,
and denote the standard error of $\widehat{\theta}$ by
$\textrm{SE}(\widehat{\theta})$. Then our CI is
...
thus insist on the more awkward phrasing, so as to avoid students
misunderstanding. Indeed some instructors would contend that use of the
word *in* is itself just plain incorrect. My own view is that in some
cases the word *in* is clearer (and certainly correct in any case), and
that it is better to add a warning about what is random/nonrandom than
engage in awkward phrasing.
