...
[In general, for confidence level $1-\alpha$ replace 1.96 by the upper
$1-\alpha/2$ quantile of the N(0,1) distribution]{.column-margin} A,
say 95%, confidence interval (CI) for $\mu$ is then
...
computed estimator, e.g. for the estimated coefficients vector in a
...
functions have this for their output, including **lm**. When we make the
...
[Often $ACov(v,w)$ will be provided by our application software, such as
with R's **vcov** function to be presented later in this
chapter.]{.column-margin} One must be careful, making sure we choose the
appropriate quantity in $\widehat{\Sigma}$. For instance, in our ratio
example here, 
...
To obtain $ACov(v,w)$, let's first look at the covariance matrix $\Sigma$,
...
concept is known as *multiple inference* or *simultaneous inference*.

Whether to do this is a philosphical question, and the answer will
depend on one's goals and personal preferences, or possibly due to
requirements of a research journal or employer. If we do wish to pursue
...
distribution.]{.column-margin} It can be shown that if $W$ is a
$k$-dimensional normally distributed random vector with mean vector
$\mu$ and covariance matrix $\Sigma$, then

$$
Q = (W-\mu)' \Sigma^{-1} (W-\mu)
$$

has a *chi-square* distribution with $k$ *degrees of freedom*. Say $d_{\alpha}$ 
is the upper-$\alpha$ quantile of that distribution, i.e.

$$
P[(W-\mu)' \Sigma^{-1} (W-\mu) \leq d_{\alpha}] = 1-\alpha
$$

In the case $k=2$, the set of all $t$ such that

$$
(t-\mu)' \Sigma^{-1} (t-\mu) = r
$$

is an ellipse. (If $\Sigma = I$, we have a circle.) For $k=3$ we have an
ellipsoid, a "football," and so on in higher dimensions..  [We will use
the linear model here for concretenss, but the same analysis holds for
any asymptotically normal statistical estimator, e.g.  Maximum
Likelihood estimates.]{.column-margin} Now, in the context of the linear
model, take $W$ to be $\widehat{\beta}$, $\mu = \beta$ etc.  

[The F-distribution is defined to be the distribution of the ratio of
two independent chi-square random variables, each divided by their
degrees of freedom. Here $r_{\alpha} = p f_{alpha}$, where $f_{\alpha}$
is the upper-$\alpha$ quantile for the F-distribution with $p$ and $n-p$
degrees of freedom.]{.column-margin} Here $\Sigma$ must be estimated by
$\widehat{\beta}$ (given to us via **vcov**), and the chi-square
distribution is replaced by a quantity involving the *F-distribution*.
using a new cutoff value of $r_{\alpha}$ rather than $d_{\alpha}$. 
...
has probability $1-\alpha$. Equivalently,
...
By the way, note that the larger $\alpha$ is, the larger will be the
value of $r_{\alpha}$, thus the larger the ellipse. The reader should
pause to confirm that this makes sense.

However, this is but an itermediate step toward our goal of a multiple
inference method.
...
Substituting into @eq-constraint, we find that 
...
boundary) is at odds with the Langrange formulation, which stipulated
that the min or max values are on the boundary. But the latter property
is implied by the lineariryt of $a'\beta$: Consider any point $q$ that
...
$a'\beta$, find its maximum and minimum values subject to $\beta$ being
in the ellipsoid In other words, we solve the Lagrange multiplier problem

$$
min, max_{\beta, \lambda} ~~
a'\beta + \lambda [(\beta - \widehat{\beta})' 
\widehat{\Sigma}^{-1} (\beta-\widehat{\beta}) - r_{\alpha}] 
$${#eq-scheffeobjftn}
...
$$
2 \lambda = \pm \sqrt{\frac{1}{r_{\alpha}} a'\widehat{\Sigma} a}
$$
...
$$
a'\beta = a'\widehat{\beta} \pm
\sqrt{r_{\alpha} a'\widehat{\Sigma} a}
$${#eq-scheffeci}

Now, why does this interval hold *simultaneously* over *all* $a$?
The point is that if \beta is in the ellipsoid, then @eq-scheffeci
will hold for all $a$. 

This analysis also shows that @eq-scheffeci is *conservative*,
with an individual probability of more than $100(1-\alpha)$
