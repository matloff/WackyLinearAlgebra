...
$\beta$ being *in* the ellipse" (i.e. either in the interior or on the
...
is strictly interior to the ellipse, so our objective function has value
$a'q$.  Then there is room for us to move away from $q$ yet still be
inside the ellipse, say to the point $u$, yet with $a'u$ either larger
...
in the ellipse. In other words, we solve the Lagrange multiplier problem
