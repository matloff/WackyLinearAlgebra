...
It can be shown that if $W$ is a $k$-dimensional normally distributed
random vector with mean vector $\mu$ and covariance matrix $\Sigma$, then
...
Maximum Likelihood estimates. By the way, here we must assume the
classical linear model assumptions, i.e. normality, linearity and
homoskedasticy.]{.column-margin} Now, in the context of the linear
model, take $W$ to be $\widehat{\beta}$, $\mu = \beta$ etc.  Here
$\Sigma$ must be estimated by $\widehat{\beta}$ (given to us via
**vcov**), and the chi-square distribution is replaced by something
called the *F-distribution*, using $f_{mlo}$ rather than $d_{\alpha}$,
