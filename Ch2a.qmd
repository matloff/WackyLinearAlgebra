...
# Covariance Matrices, MV Normal Distribution, Delta Method
...
* Matrix form of definition: 
  ...
  so $(X - X) (X - EX)'$ is $k \textrm{ x } k$.  The expected value is
...
* Say we have a random vector $X$, of length $k$, and a nonrandom matrix
  $A$ of size $m \textrm{ x } k$. Then $A X$ is a new random vector $Y$ of $m$
...
## Cramer-Wold Theorem {#thm-mvuvnormal}
...
dummies, $AVar(v,w)$ is $12 \textrm{ x } 12$. This is the covariance
