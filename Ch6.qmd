...
of $\widehat{\beta}$ with maximal variance

Recall from @sec-varbhat that in the homogeneous-variance case, the
covariance matrix of $\widehat{\beta}$ is equal to 
...
$$
Var(u'\widehat{\beta}) = \sigma^2 u'(A'A)^{-1}u
$$
...
problems, by adding a new variable corresponding to the constraint. In
our case, we minimize
