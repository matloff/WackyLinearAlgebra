...
values of $u$, no maximum.  So, let's constrain it to vectors $u$
of length 1:
...
$$
\sigma^2 (A'A)^{-1} u = -\gamma u
$$

Aha! The vector $u$ is an eigenvector of the matrix $\sigma^2
(A'A)^{-1}$ with eigenvalue $-\gamma$. And a little simpler, $u$ is an
eigenvector of $(A'A)^{-1}$ with eigenvalue $-\gamma/\sigma^2$.

We need not know the values of $\gamma$ and $\sigma^2$ here, as we can
find the eigenvalues and eigenvectors of $(A'A)^{-1}$ directly.  We've
discovered that our quest for dimension reduction can be couched as an
...
proceed. For convenience, we will also remove the nonnumeric columns,
**PIDN** and **Depth**.
...
At this point, we could call **cov()}** and then **eigen** to perform
the eigenanalysis, but R's **prcomp** function does all the work for us.
Again, for convenience, let's just apply the concept to the $X$ data;
out goal is simply dimension reduction, a rough, informal action,
