...
Algebra Done Right* is that determinants are overemphasized. He
...
The formation of the terms in general, and the deterimination of 
...
It involves sums and differences of permuted products of
distinct elements of the matrix.
...
$$
W_n = \sqrt{n} \Sigma^{-1} (\bar{X} - \mu)
$$

(Be sure to take a moment to verify the compatibility of the matrix
multiplication here. Say the $X_i$ each have length $q$. Then
$\Sigma$ is $q \textrm{ x } q$, as is its inverse, while
$\bar{X}$ and $\mu$ are also of length $q$, i.e.\ $q \textrm{ x } 1$
as matrices.)
...
Recall that a \textit{multinomial} random vector is the mathematial
...
Let's find $Cov(X)$. Define the \textit{indicator} vector 
...
Here $I_ij$ is 1 or 0, depending on whether trial $i$ resulted in
...
This is one of the most useful simple tools in statistics.  As an
example, say we are estimating a population mean $\mu$ and are also
interested in estimating $\log(\mu)$. 

We will probably use the sample mean $\bar{X}$ to estimate $\mu$, and
thus use $\log{\bar{X}}$ to estimate $\log(\mu)$. 
[If we just need to form a confidence
interval for $\log(\mu)$, we can form a CI for $\mu$ and then take the
log of both endpoints.]{.column-margin}
But how do we obtain a standard error for the latter?

(Recall that roughly speaking, the term \textit{standard error} of
an estimator $\widehat{\theta}$ of some population parameter $\theta$
is our estimate of $\sqrt{\widehat{\theta}}$. This can be used to form a
confidence interval, but also stands on its own as an indication of the
accuracy of $\widehat{\theta}$[Technically, $\widehat{\theta}$ needs to
be asymptotically normal, in which case the standard error is an
estimate of the standard deviation of that distribution.]{.column-margin})
...
In our case here, setting $W = \log{\bar{X}}$
and $f(t) = \log{t}$, we have
...
Now, what if the function $f$ has two arguments instead of one?
