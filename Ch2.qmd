...
case by peforming a $w'w$ operation, albeit with $^{-k/2}$ in the
middle. 

Clearly, we should not stretch these analogies very far, but they do
help our intuition here.

### Implication for classical statistics
...
We will probably use $\bar{X}$ to estimate $\mu$, and thus use
$\log{\bar{X}}$ to estimate $\log(\mu)$. But how do we obtain a standard
error for the latter?[If we just need to form a confidence interval for
$\log(\mu)$, we can form a CI for $\mu$ and then take the log of both
endpoints.]{.column-margin}
...
In our case here, setting $W = \log{\bar{X}}$, we have
