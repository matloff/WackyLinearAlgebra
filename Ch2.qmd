...
Let's find an expression for $g$, the distribution of $X_1$.
...
> For any square matrix $A$, it's *inverse* $B$ (if it exists)
...
consists of all 0s, there is no way to get $I$ for $AB$.
...
Recall @eq-stationMarkov, which expresses the vector of long-run state
probabilities for a Markov chain with transition matrix P and stationary
distribution $\nu$,
...
If our matrix $G$ is invertible, we can premultiply both sides of our
...
We'll go into this further in @sec-covar.
