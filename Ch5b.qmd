...
An $n ~ \textrm{x} ~ n$ symmetric matrix $M$ is called *positive
definite* if 
...
for all nonzero vectors $w$. (And the term used is *nonnegative
definite* if $>$ is replaced by $ \geq$.) Show that $<u,v> = u'Mv$ is an
inner product on $\cal R^n$.)
...
❄️  **Your Turn:** Consider the space $\cal RV(\Omega)$.  Show that if
$<X,X> = 0$, then $X$ must be the 0 vector. Make use of the facts that
...
❄️  **your turn:** show that @eq-hii holds. holds. holds.
