...
to consist only of mean-0 random variables. 
Note by the way that any random variable $X$ has a mean-0 "partner," in
the form of $\tilde{X} = X - E(X)$. This fact will come in handy at
various points in the succeeding chapters.]{.column-margin}
...
Consider random variables $X$ and $Y$ in $\cal RV(\Omega)$, and consider
...
case here.  Our goal is to eliminate the influence of $S$.
...
earlier, all variables are assumed centered, by subtracting their means,
so that they now have mean 0.
