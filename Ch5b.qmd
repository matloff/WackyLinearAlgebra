...
defendant would recividate. It is a commercial product by Northpointe.

However, COMPAS came under intense scrutiny after ' [an
investigation](https://www.propublica.org/article/machine-bias-risk-assessments-in-criminal-sentencing)
by *ProPublica*, which revealed evidence of racial bias against black
...
treated black and white defendants equally.  While this book does not
take a position on the specific dispute, this case highlights the
critical importance of addressing fairness in machine learning.
...
using $m$ dichotomous variables with regards to the material presented
here. We will consider only the numeric case here.
...
Recall that quantities like $E(X|S)$ are projections. That implies 
that the residuals, e.g. $U$ above, are orthogonal to the projections,
i.e. have correlation 0:
