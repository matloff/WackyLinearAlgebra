...
lengthy. It is needed for the section on fairness in
machine learning, @sec-fairness, which hopefully makes it worthwhile.
But it may be safely skipped, not needed for the non-$\cal RV(\Omega)$ 
parts of the book.]{.column-margin}
...
What about $E(Y | B = 0)$? A bit more involved, but still
straightforward. If $B = 0$, then $X$ was 1, 2, 3, 4 or 5, with
probability 1/5 each, mean of 3. So
...
space $\cal RV(\Omega)$. The theorem talks of a closest vector $C$ in
$W$ to $Y$. Let's see what form $C$ might take.
...
The minimum value of $E[(Q-d)^2]$ is attained by $d = E(Q)$. (See Your
Turn problem below.)  So, the minimum of $E((Y-C)^2 | C)$, for all
random variables $C$, is attained by the conditional mean,
...
The projection is the conditional mean.
