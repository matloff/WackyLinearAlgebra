...
> and $U$ has dimension $r \textrm{ x } (n-r)$.

It will turn out the $r$ is the rank of $A$.
...
AW$ are equal to that of $A$.
:::

::: {.proof}
Say $A$ has rank $r$. Then there exist $r$ (and no more than $r$)
linearly independent rows of $A$. For convenience of notation, say 
...
the vectors $Vc_i$ are linearly independent, $VA$ has rank $r$.
