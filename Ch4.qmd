...
the matrix has *rank* 4 -- 4 nonzero rows --  when it needs to be 5.  We
still have not formally defined rank, just building intuition, but
toward that end, let us first formalize the row operations process.
...
> \right )
> $${#eq-rref}
> where $I_s$ is an identity matrix of size $s$
> and $U$ has dimension $s \textrm{ x } (n-s)$.

Clearly, the rank of $$

It will turn out the $s$ is the rank of $A$.
...
Note that a set of vectors is linearly dependent if and only if one of
...
which an inverse is necessary (or must be worked-around). 
...
## Rank in Relation to Sets of Linear Dependent/Independent Vectors

Here we use rank as our entre'e to key concepts about sets of vectors.
...
the vectors $Vc_i$ are linearly independent, $VA$ has column rank $r$.
...
EACH ELEMENTARY ROW OP LEAVES ROW RANK UNCHANGED. SAY WE HAVE A NONZRO LIN COM
LAMBDA_I ETC. SHOW HOW EACH OF THE 3 KINDS OF ROW  OPS JUST CHANGES THE
LAMBDAS.
