...
## The application of an elementary row operation on a matrix will will leave the row rank unchanged.
:::
...
for any of the three elementary operations, the *same* linear
combination works with the modified matrix:
...
* Add *c* times row $j$ to row $i$: Set $a_i \rightarrow a_i - c a_j$.
...
The lemma shows that the row rank of $A$ is the same as that of $A_{rref}$.
But that is $s$ in @eq-rref, which we found earlier was equal to the
column rank of $A$.
