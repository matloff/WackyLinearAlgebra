...
A matrix times a vector equals a constant times that same vector.  We
say that $-\gamma$ is an *eigenvalue* of the matrix $Cov(X)$, and that
$u$ is a corresponding *eigenvector*.  Seems innocuous, but it opens a
huge new world! We will return to the notion of principal components in
the next chapter, after laying the groundwork in the current chapter.
